...
assert driver.find_element(By.XPATH, "//*[@data-test = 'error' and contains(text(), 'Username and password do not match any user in this service' )]").is_displayed()

## Negative assertion of product page.

# Define a custom expected condition for invisibility of the element
def element_not_present(driver):
    try:
        return not driver.find_element(By.XPATH, "//span[@class = 'title']").is_displayed()
    except NoSuchElementException:
        return True

assert element_not_present(driver)

driver.find_element(By.XPATH, "//div[@class = 'login_logo']").is_displayed()
