...
def user_is_logged_in_with_valid_credentials(self, browser: WebDriver):
    # user logs in
...
def the_user_adds_random_items_to_cart(browser: WebDriver):
     # Adding random items to cart
    products_page = ProductsPage(browser)
    products_page.add_random_nr_items_to_cart()

@given('the user has navigated to the cart')
def the_user_has_navigated_to_the_cart(browser):
    products_page = ProductsPage(browser)
